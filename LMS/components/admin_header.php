<?php
include_once 'connect.php'; // Include the file containing database connection code
...
<header class="header">
    <section class="flex">
        ...
        <div class="profile">
            ...
            $select_profile_query = "SELECT * FROM tutors WHERE id = $1";
            $select_profile_result = pg_query_params($conn, $select_profile_query, array($tutor_id));
            if ($select_profile_result && pg_num_rows($select_profile_result) > 0) {
...
<div class="side-bar">
    ...
    <div class="profile">
        <?php
        // Fetch profile information for sidebar
        $select_profile_query = "SELECT * FROM tutors WHERE id = $1";
        $select_profile_result = pg_query_params($conn, $select_profile_query, array($tutor_id));
        if ($select_profile_result && pg_num_rows($select_profile_result) > 0) {
            $fetch_profile = pg_fetch_assoc($select_profile_result);
