...
if(isset($_POST['update'])){
   ...
   $update_content_query = "UPDATE content SET title = $1, description = $2, status = $3 WHERE id = $4";
   $update_content_result = pg_query_params($conn, $update_content_query, array($title, $description, $status, $video_id));

   if(!empty($playlist)){
      $update_playlist_query = "UPDATE content SET playlist_id = $1 WHERE id = $2";
      $update_playlist_result = pg_query_params($conn, $update_playlist_query, array($playlist, $video_id));
   }
   ...
   if(!empty($thumb)){
      if($thumb_size > 2000000){
         ...
         $update_thumb_query = "UPDATE content SET thumb = $1 WHERE id = $2";
         $update_thumb_result = pg_query_params($conn, $update_thumb_query, array($rename_thumb, $video_id));
   ...
   if(!empty($video)){
      $update_video_query = "UPDATE content SET video = $1 WHERE id = $2";
      $update_video_result = pg_query_params($conn, $update_video_query, array($rename_video, $video_id));
...
<section class="video-form">
   ...
   <?php
      $select_videos_query = "SELECT * FROM content WHERE id = $1 AND tutor_id = $2";
      $select_videos_result = pg_query_params($conn, $select_videos_query, array($get_id, $tutor_id));
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <select name="playlist" class="box">
         ...
         if(pg_num_rows($select_playlists_result) > 0){
            while($fetch_playlist = pg_fetch_assoc($select_playlists_result)){
         ?>
