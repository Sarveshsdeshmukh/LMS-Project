...
if(isset($_POST['like_content'])){
   ...
   if($user_id != ''){
      ...
      $select_content = pg_prepare($conn, "select_content", "SELECT * FROM content WHERE id = $1 LIMIT 1");
      if (!$select_content) {
         $message[] = 'Error preparing select_content statement';
      } else {
         $select_content_result = pg_execute($conn, "select_content", array($content_id));
         if (!$select_content_result) {
            ...
            $select_likes = pg_prepare($conn, "select_likes", "SELECT * FROM likes WHERE user_id = $1 AND content_id = $2");
            $select_likes_result = pg_execute($conn, "select_likes", array($user_id, $content_id));
   ...
   <?php
      ...
      if($select_content_result !== false && pg_num_rows($select_content_result) > 0){
         while($fetch_content = pg_fetch_assoc($select_content_result)){
            ...
            $select_likes = pg_prepare($conn, "select_likes", "SELECT * FROM likes WHERE content_id = $1");
            $select_likes_result = pg_execute($conn, "select_likes", array($content_id));
